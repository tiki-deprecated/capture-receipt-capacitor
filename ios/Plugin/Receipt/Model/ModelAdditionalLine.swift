 ...
 */
class ModelAdditionalLine {
    /// The type of the additional line (e.g., "Subtotal").
    private let type: ModelStringType?
    
    /// The text content of the additional line.
    private let text: ModelStringType?
    ...
    init(additionalLine: BRProductAdditionalLine) {
        type = ModelStringType(stringType: additionalLine.type)
        text = ModelStringType(stringType: additionalLine.text)
